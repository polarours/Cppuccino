...
逻辑状态的划分本质上是**设计者的主观决定**，而不是语言或编译器强制规定。

常见的经验法则是：
- 如果这个成员的变动会让**外部观察者感知到对象“变了”** → 属于逻辑状态
- 如果变动**纯粹是实现细节、缓存、调试信息、统计计数**等，外部使用者不应该依赖它 → 可以归为物理状态
- 最经典的 mutable 使用场景，就是允许在 `const` 方法中修改物理状态，而不违反逻辑状态的不变性。
...
> 以上的例子展示的是 mutable 的正向使用场景，当然 mutable 也可能被滥用，导致代码难以理解和维护。后续的章节（待补充）会讨论 mutable 的反面案例以及说明为什么需要使用 mutable。
