...
`std::move` 会显式地声明：该对象在此之后不再承担原有语义责任。
...
> **提示**: 关于左值引用和右值引用的详细解释，请参考[左值引用和右值引用](/docs/中文版/左值引用和右值引用.md)章节。
...
> 它等价于一个 `static_cast<T&&>`（更准确说是 cast 成一个 “去引用后的 `T` 的右值引用”）。
> 注意：对 `const T` 使用 `std::move` 通常不会触发移动构造，因为移动构造函数一般不能接受 `const T&&`。
...
> 它只是告诉编译器：“我允许你把当前的左值，当成一个即将被放弃的对象来看待。”
...
后来为了改变这种行为，C++11 引入了右值引用（rvalue reference），但是只有通过转换——也就是移动语义，才能将一个左值变成右值。这就是 `std::move` 存在的原因。
...
3. `str` 现在处于一个有效但未指定的状态（通常是空字符串）。

## move 之后的对象：为什么是“有效但未指定状态”

使用 `std::move` 后，原对象的状态取决于其类型的移动语义：
- 对于标准库类型（如 `std::string`、`std::vector` 等），移动后通常会将资源的所有权转移到新对象，原对象变为空或处于某种“有效但未指定”的状态。
...
我们来总结一下 `std::move` 的关键点：
- `std::move` 本身不会改变对象状态，它只允许移动语义发生。原对象仍然有效，但状态可能未指定或为空，取决于类型的移动实现。
